...
namespace NNLibrary
{
    public class Matrix
    {
        ...
        public void EditElement(int row, int column, double e) { array[row, column] = e; }

        public double GetElement(int row, int column) { return array[row, column]; }
        ...
        public Matrix ToVectorOfRows_horizontal()
        {
            Matrix m = new Matrix(1, row * column);
            int k = 0;
            for (int i = 0; i < row; i++)
            {
                for (int j = 0; j < column; j++)
                {
                    m.array[0, k] = array[i, j]; k++;
                }
            }
            return m;
        }

        public Matrix ToVectorOfColumns_vertical()
        {
            Matrix m = new Matrix(row * column, 1);
            int k = 0;
            for (int i = 0; i < column; i++)
            {
                for (int j = 0; j < row; j++)
                {
                    m.array[k, 0] = array[j, i]; k++;
                }
            }
            return m;
        }
        ...
        public static Matrix Sigmoid(Matrix m1)
        {
            ...
            for (int i = 0; i < m.Row; ++i)
            {
                ...
                {
                    m.array[i, j] = sigmoid(m1.array[i, j]);
                }
            }
            return m;
        }

        public static Matrix DSigmoid(Matrix m1)
        {
            Matrix m = new Matrix(m1.row, m1.column);
        ...
        public static Matrix softmax(Matrix m1)
        {
            Matrix m = new Matrix(m1.row, m1.column);
            double sum = 0;
            for (int i = 0; i < m.Row; ++i)
            ...
            for (int i = 0; i < m.Row; ++i)
            {
                for (int j = 0; j < m.Column; ++j)
                {
                    m.array[i, j] = Math.Exp(m1.array[i, j]) / sum;
                }
            }
            return m;
        }
        ...
        public static Matrix operator &(Matrix m1, Matrix m2)
        {
            ...
            {
                throw new Exception("Почленное умножение невозможно");
            }
        ...
        public static Matrix operator *(double alpha, Matrix m2)
        {
            Matrix m = new Matrix(m2.row, m2.column);

            for (int i = 0; i < m2.row; i++)
            {
                for (int j = 0; j < m2.column; j++)
                {
                    m.array[i, j] = m2.array[i, j] * alpha;
                }
            }
            return m;
        }
